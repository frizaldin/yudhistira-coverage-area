...
export default function TrlDetail({ activeNav, cabangName, cabangCode, backUrl, targetYear, prevYear, summary = {}, trlBySales = [], sekolah = {}, salesList = [], jenjangList = [], filters = {} }) {
    const [activeTab, setActiveTab] = useState("trl-sales");
    const fmt = (n) => Number(n || 0).toLocaleString("id-ID");

    const apply = (key, val) => {
        router.get(window.location.pathname, { ...filters, [key]: val || undefined, page: undefined }, { preserveState: true, preserveScroll: true, replace: true });
    };
    const goPage = (p) => router.get(window.location.pathname, { ...filters, page: p }, { preserveState: true, preserveScroll: true });
    const resetFilters = () => router.get(window.location.pathname, {}, { replace: true });
    const hasFilter = filters.sales_id || filters.jenjang || filters.trl_status;

    const salesGroups = trlBySales.reduce((acc, row) => {
        if (!acc[row.sales_id]) acc[row.sales_id] = { sales_id: row.sales_id, sales_name: row.sales_name, rows: [] };
    ...
    return (
        <MonitoringLayout activeNav={activeNav}>
            <Head title={`TRL Detail – Cabang ${cabangName}`} />
            <div style={{ padding: "20px 24px", maxWidth: 1320, margin: "0 auto" }}>

                {/* Header */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
                    <div>
                        <Link href={backUrl} style={{ color: T.blue, textDecoration: "none", fontSize: 12.5, fontWeight: 600, display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
                        ...
                        <h1 style={{ fontSize: 22, fontWeight: 800, color: T.text, margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
                            <span style={{ background: "linear-gradient(135deg,#1d4ed8,#7c3aed)", padding: "4px 10px", borderRadius: 8, display: "inline-flex" }}>
                                <i className="bi bi-shield-shaded" style={{ color: "white", fontSize: 16 }} />
                            </span>
                            TRL Detail — {cabangName}
                        </h1>
                        <p style={{ fontSize: 12, color: T.slate, margin: "5px 0 0 0" }}>
                            Tahan · Rebut · Lepas · Gagal per Sales per Segmen &amp; List Sekolah &nbsp;·&nbsp; {prevYear} → {targetYear}
                        </p>
                    </div>
                    {hasFilter && (
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                            {filters.trl_status && (
                                <span style={{ background: TRL_META[filters.trl_status]?.bg, color: TRL_META[filters.trl_status]?.color, padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>
                                    {TRL_META[filters.trl_status]?.label}
                                    <button onClick={() => apply("trl_status", "")} style={{ border: "none", background: "transparent", cursor: "pointer", color: "inherit", marginLeft: 4, fontSize: 13, padding: 0 }}>×</button>
                                </span>
                            )}
                            {filters.jenjang && (
                                <span style={{ background: `${JENJANG_COLOR[filters.jenjang] || T.slate}18`, color: JENJANG_COLOR[filters.jenjang] || T.slate, padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>
                                    {filters.jenjang}
                                    <button onClick={() => apply("jenjang", "")} style={{ border: "none", background: "transparent", cursor: "pointer", color: "inherit", marginLeft: 4, fontSize: 13, padding: 0 }}>×</button>
                                </span>
                            )}
                            <button onClick={resetFilters} style={{ padding: "4px 10px", borderRadius: 20, border: "1px solid #fca5a5", background: "#fee2e2", color: T.red, fontSize: 10.5, fontWeight: 600, cursor: "pointer" }}>Reset Semua</button>
                        </div>
                    )}
                </div>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
                    {["tahan", "rebut", "lepas", "gagal"].map((s) => (
                        <SummaryCard key={s} status={s} value={summary[s] ?? 0}
                            active={filters.trl_status === s}
                            onClick={() => { apply("trl_status", filters.trl_status === s ? "" : s); setActiveTab("sekolah"); }} />
                    ))}
                </div>

                {/* Filter Bar */}
                <div style={{ ...S.card, padding: "10px 14px", marginBottom: 12, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                    <select value={filters.sales_id || ""} onChange={(e) => apply("sales_id", e.target.value)}
                        style={{ fontSize: 12, padding: "5px 10px", borderRadius: 7, border: `1px solid ${T.border}`, background: "white", minWidth: 160 }}>
                        <option value="">Semua Sales</option>
                        {salesList.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                    <select value={filters.jenjang || ""} onChange={(e) => apply("jenjang", e.target.value)}
                        style={{ fontSize: 12, padding: "5px 10px", borderRadius: 7, border: `1px solid ${T.border}`, background: "white" }}>
                        <option value="">Semua Jenjang</option>
                        {jenjangList.map((j) => <option key={j} value={j}>{j}</option>)}
                    </select>
                    <select value={filters.trl_status || ""} onChange={(e) => apply("trl_status", e.target.value)}
                        style={{ fontSize: 12, padding: "5px 10px", borderRadius: 7, border: `1px solid ${T.border}`, background: "white" }}>
                        <option value="">Semua Status TRL</option>
                        {["tahan", "rebut", "lepas", "gagal"].map((s) => <option key={s} value={s}>{TRL_META[s].label}</option>)}
                    </select>
                    <div style={{ marginLeft: "auto", fontSize: 11, color: T.slate }}>
                        <i className="bi bi-funnel" style={{ marginRight: 4 }} />{hasFilter ? "Filter aktif" : "Tidak ada filter"}
                    </div>
                </div>

                {/* Tab Navigation */}
                <div style={{ display: "flex", gap: 2, marginBottom: 12, background: "#f1f5f9", padding: 4, borderRadius: 10, width: "fit-content" }}>
                    {[{ id: "trl-sales", label: "TRL per Sales per Segmen", icon: "bi-person-lines-fill" }, { id: "sekolah", label: `List Sekolah (${sekolah?.total ?? 0})`, icon: "bi-building" }].map((tab) => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "7px 14px", borderRadius: 7, border: "none", background: activeTab === tab.id ? "white" : "transparent", color: activeTab === tab.id ? T.text : T.slate, fontWeight: activeTab === tab.id ? 700 : 500, fontSize: 12, cursor: "pointer", boxShadow: activeTab === tab.id ? "0 1px 4px rgba(15,23,42,0.08)" : "none", display: "flex", alignItems: "center", gap: 6 }}>
                            <i className={`bi ${tab.icon}`} />{tab.label}
                        </button>
                    ))}
                </div>

                {/* TAB 1: TRL per Sales per Segmen */}
                {activeTab === "trl-sales" && (
                            ...
                            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                                ...
                                <tbody>
                                    {sekolah?.data?.length > 0 ? sekolah.data.map((s, i) => (
                                        ...
                                        <tr>
                                            <td colSpan="9" style={{ ...S.td, textAlign: "center", padding: "40px 0", color: T.slate }}>

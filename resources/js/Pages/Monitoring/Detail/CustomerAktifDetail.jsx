...
export default function CustomerAktifDetail({ activeNav, scopeName, backUrl, customers, filters, kecamatanList, jenjangList }) {
    ...
    return (
        <MonitoringLayout activeNav={activeNav}>
            <Head title={`Customer Aktif - ${scopeName}`} />
            <div style={{ padding:'20px 24px', maxWidth:1200, margin:'0 auto' }}>
                ...
                <div style={{ marginBottom:20 }}>
                    ...
                    <h1 style={{ fontSize:22, fontWeight:800, color:T.text, margin:'8px 0 4px' }}>
                        Customer Aktif — {scopeName}
                    </h1>
                    <div style={{ fontSize:12, color:T.slate }}>
                        Sekolah yang sudah aktif sebagai customer di wilayah ini
                    </div>
                </div>
                ...
                <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
                    {[
                        { label:'Total Customer', value:fmt(customers.total), color:T.green, icon:'bi-people-fill' },
                        { label:'Halaman', value:`${customers.current_page}/${customers.last_page}`, color:T.slate, icon:'bi-file-earmark-text' },
                    ].map((s,i)=>(
                ...
                <div style={{ ...S.card, padding:'12px 16px', marginBottom:12, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    ...
                    <div style={{ fontSize:11, color:T.slate }}>
                        Menampilkan {customers.from}–{customers.to} dari {fmt(customers.total)} customer
                    </div>
                </div>
                ...
                <div style={{ ...S.card, overflow:'hidden' }}>
                    <table style={{ width:'100%', borderCollapse:'collapse' }}>
                        <thead>
                            <tr>
                                {['No','Nama Sekolah','Kecamatan','Jenjang','Total Siswa'].map(h=>(
                                    <th key={h} style={{ ...S.th, textAlign:h==='Total Siswa'?'right':'left' }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {customers.data?.map((s,i)=>(
                                <tr key={s.id} style={{ background:i%2===0?'white':'#fafbfc' }}>
                                    <td style={{ ...S.td, width:44, color:T.slate }}>{(customers.from||1)+i}</td>
                                    ...
                                    <td style={{ ...S.td, textAlign:'right', fontWeight:700 }}>{fmt(s.total_student)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    <Pagination meta={customers} onPage={goPage} />
                </div>
            </div>
        </MonitoringLayout>
    );
